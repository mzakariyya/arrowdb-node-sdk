...
function createArrowDBRequestFunction(options) {
	...
	return function (restOptions, callback) {
		...
		function prepareArrowDBRequest() {
			...
			console.log("slashNeeded", slashNeeded)
			var apiEntryPoint = this.appOptions.apiEntryPoint + '/v2/' + options.arrowDBObjectName + slashNeeded + arrowDBMethod + '.json?key=' + this.appKey;
