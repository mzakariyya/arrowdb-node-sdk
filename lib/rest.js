...
const
	...
	userLoginParams = ['_login', '_password', '_admin_']
	blankMethods = ['create', 'delete', 'get', 'update']
	manyMethods = ['createMany', 'deleteMany', 'updateMany']
 ...
 * @param {string} arrowDBMethod - The Method used alongside the main component e.g. create in user/create
 ...
 */
function arrowDBRequest(apiEntryPoint, appOptions, arrowDBMethod, httpMethod, cookieString, restOptions, callback) {
	...
	if (restOptions.req) {
		...
		if (restOptions.req.headers) {
			headers = restOptions.req.headers;
		}
	...
	Object.keys(reqBody).forEach(function (item) {
		var value = reqBody[item];
			...
			} else if (userLoginParams.indexOf(item) !== -1) {
				isUser = true;
	...
	if (isUser) {
		if (arrowDBMethod != "") {
			...
			var arrayFormat
			o[arrayFormat] = []
			o[arrayFormat].push(preparedReqBody);
			preparedReqBody = JSON.stringify(o[arrayFormat]);
		}else{
			preparedReqBody= JSON.stringify(preparedReqBody);
		}
	}
 ...
 * @param {string} options.arrowdbObjectURL - v1 or v2
...
function createArrowDBRequestFunction(options) {
	...
	if (!options || !options.arrowdbObjectURL || !options.arrowDBObjectKey || !options.arrowDBObjectName  || !options.httpMethod) {
	...
	return function (restOptions, callback) {
		// parameter offset
		if (typeof restOptions === 'function') {i
		...
		function prepareArrowDBRequest() {
			...
			// remove arrowDBMethod for BAAS V2 to comply with new API's
			if (blankMethods.indexOf(arrowDBMethod) !== -1) {
				arrowDBMethod = ""
			}
			// manage arrowDBMethod for BAAS V2 to comply with new API's
			if (manyMethods.indexOf(arrowDBMethod) !== -1) {
				arrowDBMethod = arrowDBMethod.replace('Many','');
			}
			...
			if (this.dashboardSession && this.dashboardSession != '') {
				connectSID = '&connect.sid=' + this.dashboardSession
			}else{
				connectSID = '';
			}
			// Remove below statment
			// if (options.arrowdbObjectURL == 'v1'){
			// 	console.log("got v1 URL")
			// 	options.arrowdbObjectURL = 'v2'
			// }

			var apiEntryPoint = this.appOptions.apiEntryPoint + '/'+ options.arrowdbObjectURL + '/' + options.arrowDBObjectName + slashNeeded + arrowDBMethod + '?key=' + this.appKey + connectSID;
			...
			if (this.sessionCookieString && this.sessionCookieString.indexOf('=') < 0) {
				this.sessionCookieString = 'arrowDBSessionID='+encodeURIComponent(this.sessionCookieString);
			}

			arrowDBRequest(apiEntryPoint, this.appOptions, arrowDBMethod, options.httpMethod, this.sessionCookieString, restOptions, function (error, result) {
