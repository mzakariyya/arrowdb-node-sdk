...
function arrowDBRequest(apiEntryPoint, appOptions, arrowDBMethod, httpMethod, cookieString, restOptions, callback) {
	...
	Object.keys(reqBody).forEach(function (item) {
		...
		} else if (v2 &&  item == 'data') {
			preparedReqBody = value;
			return;
		} else {
...
function createArrowDBRequestFunction(options) {
	...
	return function (restOptions, callback) {
		...
		function prepareArrowDBRequest() {
			...
			var apiEntryPoint = this.appOptions.apiEntryPoint + '/'+ options.arrowdbObjectURL + '/' + options.arrowDBObjectName + slashNeeded + arrowDBMethod
			// extend to support path overload for predefined calls
			if (restOptions.path){
				apiEntryPoint += '/' + restOptions.path
				delete restOptions.path
			}
			apiEntryPoint += '?key=' + this.appKey;
